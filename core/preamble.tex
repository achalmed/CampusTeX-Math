...
\input{\CoreDir/layout}      % geometry + multicol (independiente)
\input{\CoreDir/math}        % amsmath → mathtools → unicode-math (AL FINAL de math)
\input{\CoreDir/fonts}       % selección de fuentes — REQUIERE unicode-math ya cargado
\input{\CoreDir/colors}      % xcolor + paleta — antes de todo lo que usa colores
\input{\CoreDir/boxes}       % cajas tcolorbox — usa la paleta
\input{\CoreDir/headers}     % fancyhdr — usa \NombreCurso, \NombreAcademia
\input{\CoreDir/sections}    % titlesec — usa la paleta
\input{\CoreDir/lists}       % enumitem + alternativas A–E
\input{\CoreDir/tables}      % booktabs, tabularx, multirow
\input{\CoreDir/graphics}    % tikz, pgfplots, graphicx + \graphicspath a assets/
\input{\CoreDir/links}       % hyperref — SIEMPRE después de unicode-math y titlesec
\input{\CoreDir/typography}  % microtype, setspace, parskip
\input{\CoreDir/macros}      % \ejercicio, \portadaclase — usa paleta y tikz
